...
import com.example.polls.model.audit.Vote;
...
public class PollService {
    ...
    public PagedResponse<PollResponse> getPollsCreatedBy(String username, UserPrincipal currentUser, int page, int size) {
        ...
        // Retrieve all polls created by the given user
        Pageable pageable = PageRequest.of(page, size, Sort.Direction.DESC, "createdAt");
        ...
        // Map Polls to PollsResponse containing vote counts and poll creator details
        List<Long> pollIds = polls.map(Poll::getId).getContent();
        Map<Long, Long> choiceVoteCountMap = getChoiceVoteCountMap(pollIds);
        Map<Long, Long> pollUserVoteMap = getPollUserVoteMap(currentUser, pollIds);
    ...
    public PollResponse getPollById(Long pollId, UserPrincipal currentUser) {
        ...
        // Retrieve vote doe by logged in user
        Vote userVote = null;
        if(currentUser != null) {
    ...
    public PollResponse castVoteAndGetUpdatedPoll(Long pollId, VoteRequest voteRequest, UserPrincipal currentUser) {
        ...
        // Retrieve Vote Counts of every choice belonging ro the current poll
        List<ChoiceVoteCount> votes = voteRepository.countByPollIdGroupByChoiceId(pollId);
    ...
    private Map<Long, Long> getPollUserVoteMap(UserPrincipal currentUser, List<Long> pollIds) {
        // Retrieve Votes done by the logge in user to the given pollIds
        Map<Long, Long> pollsUserVoteMap = null;
        if(currentUser != null) {
            List<Vote> userVotes = voteRepository.findByUserIdAndPollIdIn(currentUser.getId(), pollIds);

            pollsUserVoteMap = userVotes.stream()
                    .collect(Collectors.toMap(vote -> vote.getPoll().getId(), vote -> vote.getChoice().getId()));
        }

        return pollsUserVoteMap;
    }
